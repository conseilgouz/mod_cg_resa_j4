...
class mod_cg_resaInstallerScript
{
	...
	private $extname                 = 'cg_resa';
	private $previous_version        = '';
	private $dir           = null;
	private $installerName = 'cg_resainstaller';
	...
	private function postinstall_cleanup() {
		...
		sprintf("%s/modules/mod_%s/calendar.xml", JPATH_SITE, $this->extname),
		sprintf("%s/modules/mod_%s/conges.xml", JPATH_SITE, $this->extname),
		sprintf("%s/modules/mod_%s/css_admin.css", JPATH_SITE, $this->extname),
		sprintf("%s/modules/mod_%s/ouv.xml", JPATH_SITE, $this->extname)
		];
		...
		$query = $db->getQuery(true)
			->delete('#__update_sites')
			->where($db->quoteName('location') . ' like "%conseilgouz.com/updates/mod_cg_resa_update%"');
		$db->setQuery($query);
		$db->execute();
